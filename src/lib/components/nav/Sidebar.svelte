<script lang="ts">
	...
	let { user, folders, projects, open, onclose, collapsed = false, ontogglecollapse, platformName = 'PM', telnyxEnabled = false }: Props = $props();
	...
	const dialerLink = { href: '/crm/dialer', label: 'Dialer', icon: 'M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z' };

	const crmLinks = $derived(
		telnyxEnabled
			? [...baseCrmLinks.slice(0, 6), dialerLink, ...baseCrmLinks.slice(6)]
			: baseCrmLinks
	);
