<script lang="ts">
	...
	let activeTab = $state<'actions' | 'pipeline'>('actions');
...
<div class="p-6 space-y-6">
	<div class="flex items-center justify-between">
		...
		<div class="flex rounded-lg border border-surface-300 bg-surface-100 p-0.5 dark:border-surface-700 dark:bg-surface-800">
			<button
				...
				class="rounded-md px-4 py-1.5 text-xs font-medium transition {activeTab === 'actions' ? 'bg-white text-surface-900 shadow-sm dark:bg-surface-700 dark:text-surface-100' : 'text-surface-500 hover:text-surface-700 dark:hover:text-surface-300'}"
			>
				Action View
			</button>
			<button
				onclick={() => activeTab = 'pipeline'}
				class="rounded-md px-4 py-1.5 text-xs font-medium transition {activeTab === 'pipeline' ? 'bg-white text-surface-900 shadow-sm dark:bg-surface-700 dark:text-surface-100' : 'text-surface-500 hover:text-surface-700 dark:hover:text-surface-300'}"
			>
				Pipeline View
			</button>
		</div>
	...
	{#if activeTab === 'actions'}
		...
		<div class="grid gap-6 md:grid-cols-2">
			...
			<!-- Closing This Week -->
			<div class="rounded-lg border border-surface-300 bg-surface-50 p-4 dark:border-surface-800 dark:bg-surface-900">
				<h2 class="mb-3 text-sm font-semibold text-surface-900 dark:text-surface-100">Closing This Week</h2>
				{#if data.closingThisWeek.length === 0}
					<p class="text-sm text-surface-500">No deals closing this week.</p>
				{:else}
					<div class="space-y-2">
						{#each data.closingThisWeek as opp}
							<a href="/crm/opportunities/{opp.id}" class="flex items-center justify-between rounded-md p-2 transition hover:bg-surface-100 dark:hover:bg-surface-800">
								...
								<div class="ml-2 shrink-0 text-right">
									...
									{/if}
									<p class="text-[10px] text-brand-600 dark:text-brand-400">{daysUntil(opp.expectedCloseDate!)}</p>
								</div>
							</a>
						{/each}
					</div>
