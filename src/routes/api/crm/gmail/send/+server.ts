...
export const POST: RequestHandler = async (event) => {
	...
	const { to, cc, subject, html, replyToMessageId, threadId } = body as {
		to: string[];
		cc?: string[];
		subject: string;
		html: string;
	...
	try {
		...
		try {
			const { initialSync, incrementalSync } = await import('$lib/server/gmail/sync.js');
			// Quick incremental sync to pick up the sent message
